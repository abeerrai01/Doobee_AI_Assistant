...
export const LandingPage: React.FC<LandingPageProps> = ({
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-100 flex flex-col justify-between selection:bg-blue-600 selection:text-white relative overflow-hidden">
      ...
      {/* Main Hero Section */}
      <main className="w-full max-w-6xl mx-auto px-6 pt-6 pb-16 z-10 flex-1 flex flex-col items-center">
        {/* Badge Banner */}
        <motion.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-950/70 border border-blue-800/60 text-blue-300 text-xs font-semibold mb-6 backdrop-blur-md shadow-md"
        >
        ...
        <motion.h1
          ...
          className="text-4xl sm:text-6xl md:text-7xl font-extrabold text-center tracking-tight leading-[1.1] max-w-4xl"
        >
          Doobee
        </motion.h1>
        ...
        <motion.p
          ...
          className="text-lg sm:text-2xl text-slate-300 font-medium text-center mt-3 max-w-2xl leading-snug"
        >
        ...
        <motion.p
          ...
          className="text-sm text-slate-400 text-center mt-2 max-w-lg"
        >
        ...
        >
          <motion.button
            ...
            onClick={() => onStartCall()}
            disabled={isLoading}
            className="relative group px-10 py-5 rounded-3xl bg-gradient-to-r from-blue-600 via-sky-500 to-teal-400 text-white font-extrabold text-xl sm:text-2xl shadow-2xl shadow-blue-600/40 hover:shadow-blue-500/60 border border-white/20 transition-all duration-300 flex items-center gap-4 cursor-pointer disabled:opacity-75"
          >
            {isLoading ? (
              <>
              ...
              <>
                <div className="w-12 h-12 rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center shadow-inner group-hover:rotate-12 transition-transform">
                  <PhoneCall className="w-7 h-7 text-white fill-current" />
                </div>
          ...
          <div className="flex items-center gap-4 mt-4 text-xs text-slate-400">
            <span className="flex items-center gap-1">
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" /> Instant Connection
            </span>
            <span className="flex items-center gap-1">
              <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" /> Multilingual Support
            </span>
            <span className="flex items-center gap-1">
